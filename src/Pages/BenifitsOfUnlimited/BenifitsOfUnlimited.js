import React from 'react';
import './BenifitsOfUnlimited.css'
import { ImCross } from 'react-icons/im'

const BenifitsOfUnlimited = () => {
    return (
        <div>
            <div class="table">
                <div class="table-cell">
                    <h3>Benefits</h3>
                </div>
                <div class="table-cell plattform">
                    <h3>Restricted Access</h3>
                    {/* <a href="" class="btn">register now</a> */}
                </div>
                <div class="table-cell enterprise">
                    <h3>Unlimited Subscription</h3>
                    {/* <a href="" class="btn">Send us a Email</a> */}
                </div>
                <div class="table-cell cell-feature">Learn more: Get Unlimited access to 1000+ short courses</div>
                <div class="table-cell">
                    <ImCross />
                    {/* <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                        <title>check_blue</title>
                        <path d="M6.116 14.884c.488.488 1.28.488 1.768 0l10-10c.488-.488.488-1.28 0-1.768s-1.28-.488-1.768 0l-9.08 9.15-4.152-4.15c-.488-.488-1.28-.488-1.768 0s-.488 1.28 0 1.768l5 5z" fill="limegreen" fill-rule="evenodd" />
                    </svg> */}
                </div>
                <div class="table-cell">
                    <svg class="enterprise-check" width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                        <title>check_blue</title>
                        <path d="M6.116 14.884c.488.488 1.28.488 1.768 0l10-10c.488-.488.488-1.28 0-1.768s-1.28-.488-1.768 0l-9.08 9.15-4.152-4.15c-.488-.488-1.28-.488-1.768 0s-.488 1.28 0 1.768l5 5z" fill="limegreen" fill-rule="evenodd" />
                    </svg>
                </div>
                <div class="table-cell cell-feature">Learn faster: No time restrictions on course access
                </div>
                <div class="table-cell">
                    <ImCross />
                </div>
                <div class="table-cell">
                    <svg class="enterprise-check" width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                        <title>check_blue</title>
                        <path d="M6.116 14.884c.488.488 1.28.488 1.768 0l10-10c.488-.488.488-1.28 0-1.768s-1.28-.488-1.768 0l-9.08 9.15-4.152-4.15c-.488-.488-1.28-.488-1.768 0s-.488 1.28 0 1.768l5 5z" fill="limegreen" fill-rule="evenodd" />
                    </svg>
                </div>
                <div class="table-cell cell-feature">Gain digital certificates: Prove your expertise</div>
                <div class="table-cell">
                    <ImCross />
                </div>
                <div class="table-cell">
                    <svg class="enterprise-check" width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                        <title>check_blue</title>
                        <path d="M6.116 14.884c.488.488 1.28.488 1.768 0l10-10c.488-.488.488-1.28 0-1.768s-1.28-.488-1.768 0l-9.08 9.15-4.152-4.15c-.488-.488-1.28-.488-1.768 0s-.488 1.28 0 1.768l5 5z" fill="limegreen" fill-rule="evenodd" />
                    </svg>
                </div>
                <div class="table-cell cell-feature">Personalised course recommendations: Find your own path</div>
                <div class="table-cell">
                    <ImCross />
                </div>
                <div class="table-cell">
                    <svg class="enterprise-check" width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                        <title>check_blue</title>
                        <path d="M6.116 14.884c.488.488 1.28.488 1.768 0l10-10c.488-.488.488-1.28 0-1.768s-1.28-.488-1.768 0l-9.08 9.15-4.152-4.15c-.488-.488-1.28-.488-1.768 0s-.488 1.28 0 1.768l5 5z" fill="limegreen" fill-rule="evenodd" />
                    </svg>
                </div>
                <div class="table-cell cell-feature">Learn on the go: Access our brand new mobile app</div>
                <div class="table-cell">
                    <ImCross />
                </div>
                <div class="table-cell">
                    <svg class="enterprise-check" width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                        <title>check_blue</title>
                        <path d="M6.116 14.884c.488.488 1.28.488 1.768 0l10-10c.488-.488.488-1.28 0-1.768s-1.28-.488-1.768 0l-9.08 9.15-4.152-4.15c-.488-.488-1.28-.488-1.768 0s-.488 1.28 0 1.768l5 5z" fill="limegreen" />
                    </svg>
                </div>

            </div>
        </div>
    );
};

export default BenifitsOfUnlimited;